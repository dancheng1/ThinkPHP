...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
  ...
  /**
     ...
     * @return Timestamp
     * @author Created by chenzheng
     * @description 在日期上加分钟后计算出结果
     */
    public static Timestamp getSumTime(Date date, int fenzhong) {
        long times2 = fenzhong * 60 * 1000;
        if(StringUtil.isNOtNullAndBlank(date+"")){
            long times1 = date.getTime();
            long sumTimes = times1 + times2;
            return new Timestamp(sumTimes);
        }else{
            return new Timestamp(times2);
        }
    ...
    public static Timestamp getSumYear(Date date, int year) {
        Calendar calendar = new GregorianCalendar();
        calendar.setTime(date);
        calendar.add(calendar.DATE, year);//增加或者减少年
        date = calendar.getTime();   //增加或者减少后的结果
        return new Timestamp(date.getTime());
    }
     ...
     * @return Timestamp
     * @author Created by chenzheng
     * @description 根据当前给出的日期和“，”号分隔开的星期（传过来的形式 "0","1", "2", "3", "4", "5", "6"）排序成标准的按星期执行任务的星期顺序
     */
    public static String setNewWeek(Date dt, String oldWeek) {
        String str2 = "";
        String[] weekDays = oldWeek.split(",");//{"0", "1", "2", "3", "4", "5", "6"}周日 周一 周二 周三 周四 周五 周六
        ...
        for (int i = 0; i < weekDays.length; i++) {
            Integer weekDay = Integer.valueOf(weekDays[i]);
            //dt日期对应的周的天小于oldWeek中的，说明是在dt这个日期之后，也就是对应本周的天
            if (w <= weekDay) {
                lst1.add(weekDay);
            } else if (w > weekDay) {//dt日期对应的周的天大于oldWeek中的，说明是在dt这个日期之后的下一周，也就是对应下周的天
                lst2.add(weekDay);
            }
        }
        for(Integer i : lst1){
            if(StringUtils.isNullOrBlank(newWeek))
                newWeek+= i+"";
            else
                newWeek+= ","+ i+"";
        }
     ...
     * @return String
     * @author Created by chenzheng
     * @description 根据当前给出的日期算出提前的fz的时间并且格式化为Quartz
     */
    public static String formate(Date planArrivalTime, int fz) {
        String str;
        planArrivalTime = DateUtils.getSumTime(planArrivalTime, fz);
        Calendar cal = Calendar.getInstance();
        ...
        str = second + " " + minute + " " + hour + " " + day + " " + month + " " + "?" + " " + year;
        return str.trim();
    }
    /**
     * @param planArrivalTime
     ...
     * @return String
     * @author Created by chenzheng
     * @description 根据当前给出的日期获取当天的随机一个时间
     */
    public static Timestamp formateRandomDate(Date date) {
        SimpleDateFormat format1 = new SimpleDateFormat("yyyy-MM-dd 23:59:59");
        Long startDateRandom=date.getTime() ;
        Long endDateRandom=parseDate(format1.format(date)) .getTime();
        Long result=Math.round(Math.random() * (endDateRandom - startDateRandom) + startDateRandom);
